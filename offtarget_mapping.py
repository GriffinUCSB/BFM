...
'''
def plot_distance_fold_enrichment(df_list, fp):
	for i in range (len(df_list)):
		df = df_list[i]
		df2 = df.dropna()
		x = df['distance'].to_numpy()
		y = np.log(df['fold_enrichment'].to_numpy())
		plt.subplots()
		plt.scatter(x, y, label=fp[i])
		plt.legend()
		plt.xlabel("distance in kb")
		plt.ylabel("log fold_enrichment")
		plt.xlim((0,20))
		plt.savefig(fp[i] + '_plot.pdf')
'''
...
###parser.add_argument('--plots', help="to plot fold enrichment as a function of distance", action="store_true")
...
'''
if (plot == True):
	graph = plot_distance_fold_enrichment(file_list, o)
'''
